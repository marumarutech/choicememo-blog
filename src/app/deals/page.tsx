...
export default async function DealsIndex({ searchParams }: { searchParams: { page?: string } }) {
  const page = Number(searchParams.page ?? '1') || 1
  const posts = await getPostsByType('deals')
  const { items, meta } = paginate(posts, page, 10)
  return (
    <div>
      <h1 className="mb-4 text-2xl font-semibold">Deals（セール/クーポン）</h1>
      <div className="space-y-3">
        {items.map(p => <PostCard key={p.id} post={p} />)}
      </div>
      <Pagination meta={meta} base="/deals" />
    </div>
  )
}
